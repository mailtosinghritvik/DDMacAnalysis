...
if __name__ == "__main__":
    ...
    st.info(f"📁 Analyzing data from: **{file_name}**")
    ...
    with col3:
        ...
        st.metric(
            ...
            delta=f"{len(numeric_cols)} numerical"
        )
    ...
    if len(numeric_columns) >= 1:
        ...
        with col2:
            if len(numeric_columns) >= 2:
                col_x = st.selectbox("Select X-axis:", numeric_columns, key="x_axis")
                col_y = st.selectbox("Select Y-axis:", numeric_columns, index=1, key="y_axis")
                if col_x and col_y and col_x != col_y:
                    fig_scatter = px.scatter(data, x=col_x, y=col_y, title=f"{col_x} vs {col_y}")
