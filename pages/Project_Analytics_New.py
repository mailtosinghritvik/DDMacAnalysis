...
def main():
    ...
    with tab4:
        ...
        team_fig, team_df = create_project_team_allocation(api_data)
        if team_fig:
        ...
        with col1:
            ...
            if team_df is not None:
                for _, row in team_df.iterrows():
                    st.write(f"**{row['Project']}**: {row['Team Size']} people, led by {row['Lead']}")
        
        with col2:
            st.subheader("⚡ Resource Utilization")
            if team_df is not None:
                total_people = team_df['Team Size'].sum()
                total_hours = team_df['Total Hours'].sum()
                avg_hours_per_person = total_hours / total_people if total_people > 0 else 0
